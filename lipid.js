...
'use strict';
  const DEFAULT_CONFIG = {
    ...
    googletag: {
      ...
      reporting_control_values: [ 'lcid0', 'off', 't0', 't0-e0', 't0-e1', 't0-e0-ws', 't0-e0-wa', 't0-e1-ws', 't0-e1-wa' ],
  ...
  if(storedConfig) {
    ...
    if(parsedConfig.version !== DEFAULT_CONFIG.version) {
      log(level.LIPID, "Your custom stored lipid config is from an older, incompatible version. This configuration will be ignored and the default will be used.");
    }
    else {
  ...
  if (prebidQueAlreadyProcessed) {
    log(level.LIPID, `Prebid has already processed the ${hookQ} queue. LIPID may not have logged earlier config changes or events.`);
  }
  ...
  const lipidHook = () => {
    ...
    if (pbjs[hookQ].length===0 || pbjs[hookQ][0].name !== "lipidHook") {
      log(level.LIPID, "LIPID hook was not the first in the queue. LIPID may not have logged earlier config changes or events.");
    }
    const initialModuleConfig = moduleConfig();
    if(initialModuleConfig) {
      log(level.LIPID, "LiveIntent module is already configured before LIPID initialization. Double-check that the module is not enabled when CONTROL group is selected.", initialModuleConfig);
    }
            ...
            try {
              ...
              if(!targetingKeys.includes(config.googletag.reporting_key)) {
                ...
                if(liTargetingValue.length===0) {
                  ...
                  log(level.ERROR, `window.googletag.pubads().getTargeting('${config.googletag.reporting_key}') contained multiple values. Targeting has not been set correctly.`);
                }
                else {
                  // CONTROL group selected
                  if(config.googletag.reporting_control_values.includes(liTargetingValue[0])) {
                    if(!moduleIsInstalled()) {
                      log(level.INFO, `liveIntentIdSystem is not an installed module, but window.googletag.pubads().getTargeting('${config.googletag.reporting_key}') indicates CONTROL group`);
                    }
                    if(currentModuleConfig && currentConfig.userSync.syncEnabled!==false) {
                      log(level.WARNING, `window.googletag.pubads().getTargeting('${config.googletag.reporting_key}') indicates CONTROL group, but the LiveIntent module is enabled and active, so the control group may be polluted if the auction gets enriched.`);
                    }
                    else {
                      log(level.INFO, `User is in the CONTROL group. Auction was purposefully not enriched.`);
                    }
                    if (auctionWasEnriched) {
                      log(level.ERROR, `window.googletag.pubads().getTargeting('${config.googletag.reporting_key}') indicates CONTROL group, but auction was enriched. The control group was polluted.`);
                    }
                  }
                  // TREATED group selected
                    ...
                    if(!auctionWasEnriched) {
                      ...
                      if(!moduleIsInstalled()) {
                        log(level.ERROR, `window.googletag.pubads().getTargeting('${config.googletag.reporting_key}') indicates TREATED group, but liveIntentIdSystem is not an installed module. Bids will not be enriched.`);
                      }
                      if(!currentModuleConfig) {
                        log(level.ERROR, `window.googletag.pubads().getTargeting('${config.googletag.reporting_key}') indicates TREATED group, but liveIntentId module is not configured in the userSync.userIds array. Bids will not be enriched.`);
                      }
                      else {
                      ...
                      if (currentConfig.userSync.syncEnabled === false) {
                        log(level.ERROR, `window.googletag.pubads().getTargeting('${config.googletag.reporting_key}') indicates TREATED group, but the Prebid userSync is disabled. Bids will not be enriched.`, currentConfig.userSync);
                      }
                      else {
                        // Check userSync settings
                        if(currentConfig.userSync.auctionDelay===0) {
                          if(currentModuleConfig && currentModuleConfig.storage) {
                            log(level.WARNING, "userSync.auctionDelay is set to 0 at the start of the auction. However, prebid storage IS configured on the liveIntentId module, so subsequent page views may still get first auctions enriched within the expiration period.", currentModuleConfig.storage);
                          }
                          else {
                            log(level.ERROR, "userSync.auctionDelay is set to 0 at the start of the auction and Prebid storage is not configured on the LiveIntent module. Bids will not be enriched for the first auction on each page, but may be for subsequent auctions", currentModuleConfig);
                          }
                        }
                        else {
                          if(currentModuleConfig && currentModuleConfig.storage) {
                            ...
                            log(level.WARNING, `window.googletag.pubads().getTargeting('${config.googletag.reporting_key}') indicates TREATED group, but this auction was still not enriched. This may not be an error due to a mis-configuration, or it could be a transient issue due to a timeout or no identifiers returned for the user.`);
                          }
                        }
                      }
                    }
                  ...
                  // Unknown targeting value
                    ...
                    log(level.ERROR, `window.googletag.pubads().getTargeting('${config.googletag.reporting_key}') returned a value (${liTargetingValue}) that was not expected for either the TREATED or CONTROL groups. If this value is expected, please add it to the corresponding CONTROL or TREATED values array in the lipid.config and reload the page.`);
                  }
                }
              }
            }
